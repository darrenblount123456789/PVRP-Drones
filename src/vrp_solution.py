...
class VRPSolution:
    ...
    def copy(self):
        # Create a deep copy of the VRPSolution object
        return VRPSolution(
            problem=self.problem,
            solution=[route[:] for route in self.solution],  # Deep copy of the routes list
            step=self.step
        )
    ...
    def total_time(self):
        ...
        for vehicle_dests in self.solution:
            ...
            for i in range(len(vehicle_dests) - 1):
                ...
                # print(f'Prev Node {prevNode} Ready {timeIntervals[str(prevNode)][0]} Due {timeIntervals[str(prevNode)][1]} Current Node {currentNode} Ready {timeIntervals[str(currentNode)][0]} Due {timeIntervals[str(currentNode)][1]} travel {travel_time} Total Time {totalTime}')
